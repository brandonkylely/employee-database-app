...
function viewRoles(db, startApplication) {
    db.query(`SELECT role.id AS role_id, title, salary AS role_salary, department.name AS department_name FROM role LEFT JOIN department ON role.department_id = department.id`, (err, result) => {
        if (err) {
          console.log(err);
        }
